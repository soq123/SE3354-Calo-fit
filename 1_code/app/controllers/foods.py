...
def nutrition_lookup():
    ...
    try:
        ...
        return jsonify({"error": "Could not reach the nutrition database. Please try again."}), 502
    ...
    def extract(food):
        vals = {v: 0.0 for v in NUTRIENT_IDS.values()}
        for n in food.get("foodNutrients", []):
            key = NUTRIENT_IDS.get(n.get("nutrientId")) or NUTRIENT_NUMS.get(str(n.get("nutrientNumber", "")))
            if key:
                try:
    ...
    return jsonify({"results": [extract(f) for f in foods]})
